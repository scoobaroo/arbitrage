...
//			BigDecimal price = CurrencyConverter.toPrecision(exchangePrice,Main.sigDigsForPricing.get(key1+key2));
		    LimitOrder limitOrder = new LimitOrder.Builder(OrderType.BID, pair).originalAmount(amt).limitPrice(new BigDecimal(exchangePrice)).build();
...
//			BigDecimal price = CurrencyConverter.toPrecision(exchangePrice,Main.sigDigsForPricing.get(symbol));
			LimitOrder limitOrder = new LimitOrder.Builder(OrderType.ASK, pair).originalAmount(amt).limitPrice(new BigDecimal(exchangePrice)).build();
			System.out.println(limitOrder);
		    return limitOrder;
		}
	}
